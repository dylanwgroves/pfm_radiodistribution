...
/* Audio Screening 2 ___________________________________________________________*/
	...
	drop rd_treat_* 
	drop treat_*
	use  "C:\Users\grovesd\Dropbox\Wellspring Tanzania Papers\Wellspring Tanzania - Uzikwasa\01_data\pfm_uzi_cm1cm2cm3.dta", clear
...
/* Append ______________________________________________________________________*/
	...
	save "X:/Dropbox/Wellspring Tanzania Papers/Wellspring Tanzania - Radio Distribution/01 Data/pfm_rd_analysis_appendNOTclean.dta", replace

*/
**# Bookmark #7
/* Create key variables ________________________________________________________*/
	
	use "X:/Dropbox/Wellspring Tanzania Papers/Wellspring Tanzania - Radio Distribution/01 Data/pfm_rd_analysis_appendNOTclean.dta", clear
	...
	* compliance 
		...
		replace rd_working = 0 if missing(rd_working)
		replace rd_controls_self = 0 if missing(rd_controls_self)
	...
	* firststage
		...
		clonevar rd_natleader = radio_natleader 
			replace rd_natleader = e_radio_natleader if missing(rd_natleader)
			replace rd_natleader = cm3_radio_natleader if missing(rd_natleader)

	* environmental knowledge
		clonevar enviro_ccknow_short = e_env_ccknow_mean_dum
			replace enviro_ccknow_short = cm3_env_ccknow_mean_dum if missing(enviro_ccknow_short)
	...
	* gender 
		replace ge_school = e_ge_school if missing(ge_school)
		
		tab ge_work sample, m
	...
	* ipv
		clonevar ipv_reject_index = ipv_rej_disobey
			replace ipv_reject_index = e_ipv_reject_gossip if missing(ipv_reject_index)
			replace ipv_reject_index = cm3_ipv_reject_gossip if missing(ipv_reject_index)
		tab ipv_rej_disobey sample, m

		tab ipv_norm_rej sample, m

		replace ipv_report = ipv_report_index if missing(ipv_report)
		tab ipv_report sample, m
	
	* wpp 
			replace wpp_behavior = e_wpp_behavior if missing(wpp_behavior)
			replace wpp_behavior = cm3_wpp_behavior if missing(wpp_behavior)
		...
		clonevar wpp_attitude2_dum = e_wpp_attitude2_dum 
			replace wpp_attitude2_dum = cm3_wpp_attitude2_dum if missing(wpp_attitude2_dum)
	...
	* openness - neighbor 
		clonevar open_yesnbr_hiv = prej_yesnbr_aids 
			replace open_yesnbr_hiv = prej_yesneighbor_aids if missing(open_yesnbr_hiv)
			replace open_yesnbr_hiv = e_prej_yesnbr_hiv if missing(open_yesnbr_hiv)
			replace open_yesnbr_hiv = cm3_prej_yesnbr_hiv if missing(open_yesnbr_hiv)
		tab open_yesnbr_hiv sample, m

		clonevar open_yesnbr_gay = prej_yesnbr_homo
			replace open_yesnbr_gay = prej_yesneighbor_homo if missing(open_yesnbr_gay)
		tab open_yesnbr_gay sample, m
		
		clonevar open_yesnbr_alcoholic = prej_yesnbr_alcoholic 
			replace open_yesnbr_alcoholic = prej_yesneighbor_alcoholic if missing(open_yesnbr_alcoholic)
		tab open_yesnbr_alcoholic sample, m
		
		clonevar open_yesnbr_unmarried = prej_yesnbr_unmarried
			replace open_yesnbr_unmarried = prej_yesneighbor_unmarried if missing(open_yesnbr_unmarried)
			replace open_yesnbr_unmarried = e_neighbor_unmarried if missing(open_yesnbr_unmarried)
			replace open_yesnbr_unmarried = cm3_prej_yesnbr_unmarried if missing(open_yesnbr_unmarried)
		tab open_yesnbr_unmarried sample, m
		
		clonevar open_yesnbr_albino = e_neighbor_albino
			replace open_yesnbr_albino = cm3_prej_yesnbr_albino if missing(open_yesnbr_albino)
	...
	* openness - kid marry 
		clonevar open_kidmarry_difftribe = prej_kidmarry_nottribe
			replace open_kidmarry_difftribe = e_prej_kidmarry_nottribe if missing(open_kidmarry_difftribe)
			replace open_kidmarry_difftribe = cm3_prej_kidmarry_nottribe if missing(open_kidmarry_difftribe)
		tab open_kidmarry_difftribe sample, m
		
		clonevar open_kidmarry_diffrelig = prej_kidmarry_notrelig
			replace open_kidmarry_diffrelig = e_prej_kidmarry_notrelig if missing(open_kidmarry_diffrelig)
			replace open_kidmarry_diffrelig = cm3_prej_kidmarry_notrelig if missing(open_kidmarry_diffrelig)
		tab open_kidmarry_diffrelig sample, m
		
		clonevar open_kidmarry_difftz = prej_kidmarry_nottz
			replace open_kidmarry_difftz = e_prej_kidmarry_nottz if missing(open_kidmarry_difftz)
			replace open_kidmarry_difftz = cm3_prej_kidmarry_nottz if missing(open_kidmarry_difftz)
	...
	* thermometer
		gen open_thermo_notrelig = prej_thermo_muslims if b_resp_muslim != 1
			replace open_thermo_notrelig = prej_thermo_christians if b_resp_muslim == 1 & missing(open_thermo_notrelig)
			replace open_thermo_notrelig = cm2_thermo_muslims  if b_resp_muslim != 1 & missing(open_thermo_notrelig)
			replace open_thermo_notrelig = cm2_thermo_christians if b_resp_muslim == 1 & missing(open_thermo_notrelig)
			
		gen sb_sambaa = prej_thermo_sambaa if b_resp_tribe_sambaa != 1 
		gen sb_digo = prej_thermo_digo if b_resp_tribe_digo != 1 
	...
	* tzovertribe 
		recode values_tzovertribe (1 2 = 1)(3 4 5 = 0), gen(as_identity_tzovertribe_dum) 
		recode e_values_tzvstribe (1 2 = 1)(3 4 5 = 0), gen(e_identity_tzovertribe_dum) 
		recode cm3_values_tzvstribe (1 2 = 1)(3 4 5 = 0), gen(cm3_identity_tzovertribe_dum) 

		egen identity_tzovertribe_dum = rowmean(as_identity_tzovertribe_dum e_identity_tzovertribe_dum cm3_identity_tzovertribe_dum)
	
	* political participation (interest and behavior)
		drop e_ppart_interest
		recode e_pknow_interest (1 = 3 "Very")(2 = 2 "Interested")(3 = 1 "Not very")(4 = 0 "Not at all"), gen(e_ppart_interest)
		recode cm3_pknow_interest (1 = 3 "Very")(2 = 2 "Interested")(3 = 1 "Not very")(4 = 0 "Not at all"), gen(cm3_ppart_interest)
	egen ppart_interest = rowmean(ptixpart_interest e_ppart_interest cm3_ppart_interest)
	recode ppart_interest (3 = 1 "Very")(2 1 0 = 0 "Not very"), gen(ppart_interest_dum)
	
	gen ppart_collact = ptixpart_collact
	gen ppart_vote = ptixpart_vote
	gen ppart_villmeet = ptixpart_villmeet
	egen ppart_raiseissue = rowmean(e_ppart_raiseissue cm3_ptixpart_raiseissue)

	egen ppart_behavior_index = rowmean(ppart_raiseissue ptixpart_activ_voteregister ptixpart_activ_votenatl ptixpart_activ_votelocal ptixpart_activ_votetalk ptixpart_activ_workparty ptixpart_activ_rally ptixpart_activ_villmeet_share ptixpart_activ_villmeet ptixpart_activ_wardmeet ptixpart_activ_talknatlgov ptixpart_activ_collact ptixpart_activ_creategroup)
	...
	* political knowledge 
		...
		gen pknow_kenyatta = ptixknow_fopo_kenyatta
		gen pknow_justice = ptixknow_natl_justice
		gen pknow_pm = ptixknow_natl_pm
		gen pknow_em = ptixknow_em_aware 
		gen pknow_music = ptixknow_pop_music 
		gen pknow_sport = ptixknow_pop_sport 
		
		egen pknow_index = rowmean(pknow_ports pknow_vp pknow_ruto pknow_kenyatta pknow_justice pknow_pm pknow_em)
	...
	* responsibility 
		...
		recode ptixpref_responsibility (1 = 1)(2 3 4 = 0), gen(as_responsibility_vill)
		recode ptixpref_responsibility(2 3 = 1)(1 4 = 0), gen(as_responsibility_lga)
		recode ptixpref_responsibility (4 = 1)(1 2 3 = 0), gen(as_responsibility_natl)
		
		egen presponsibility_vill = rowmean(e_responsibility_vill cm3_responsibility_vill as_responsibility_vill)
		gen presponsibility_gov = (1 - presponsibility_vill)
		egen presponsibility_lga = rowmean(e_responsibility_lga cm3_responsibility_lga as_responsibility_lga)
		egen presponsibility_natl = rowmean(e_responsibility_natl cm3_responsibility_natl as_responsibility_natl)

	* ptrust 
		egen ptrust_lga = rowmean(ptixknow_trustloc e_ptixknow_trustloc cm3_ptixknow_trustloc)
		egen ptrust_natl = rowmean(ptixknow_trustnat e_ptixknow_trustnat cm3_ptixknow_trustnat)
		egen ptrust_relig = rowmean(ptixknow_trustrel e_ptixknow_trustrel cm3_ptixknow_trustrel)
	...
	* crime report (not panalysis)
		...
		gen crime_ipv_report = ipv_report 
		gen crime_em_report = em_report 
